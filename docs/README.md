...
| 27 | Endpoint `POST /payments/documents/{id}/cancel` tiada ujian — 11 ujian memanggil PaymentPort terus, jadi laluan HTTP dan semakan SP_ADMIN tidak dilindungi | Ujian controller |
| 28 | **Sekatan modul ikut pakej** belum ada. Menu diterbitkan daripada senarai keras dalam `portal-shell`, dan setiap endpoint terbuka kepada mana-mana SP. Untuk vertikal (EDU, dsb) diperlukan `sp_module_access` sebagai kebenaran SEBENAR dengan pakej sebagai template; endpoint mesti TOLAK secara lalai supaya prefix baharu tidak menjadi kebocoran senyap | Keputusan produk belum dibuat |
| 25 | **Label UI bercampur BM dan Inggeris** — 'List of Transaction' bersama 'Kod Produk'; 'Issued To' bersama 'No. Dokumen'; 'Search'/'Clear' bersama 'Tutup'. `sp_billing_setting.language` hanya digunakan dalam PDF; UI tiada mekanisme bahasa dan setiap label ditulis keras. Toggle EN/BM pada bar atas disyaki tidak berfungsi | Satu blok kerja i18n, bukan skrin demi skrin — membetulkan sambil membina bermakna menulis semula bila mekanisme masuk |
